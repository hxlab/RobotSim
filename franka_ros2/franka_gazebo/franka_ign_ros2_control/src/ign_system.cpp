...
struct jointData
{
  ...
  /// \brief ROS2 publisher for joint wrench (NEW)
  rclcpp::Publisher<geometry_msgs::msg::WrenchStamped>::SharedPtr wrench_publisher;
  
  /// \brief Flag to indicate if this joint should publish wrench data (NEW)
  ...
  int & update_rate)
{
  ...
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    ...
    // CREATE WRENCH PUBLISHERS FOR ALL JOINTS (SIMPLER APPROACH)
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    std::string joint_name = this->dataPtr->joints_[j].name;
    std::string topic_name = "/joint_wrenches/" + joint_name;
    
    this->dataPtr->joints_[j].wrench_publisher = 
      this->nh_->create_publisher<geometry_msgs::msg::WrenchStamped>(topic_name, 10);
    this->dataPtr->joints_[j].publish_wrench = true;
    
    RCLCPP_INFO_STREAM(this->nh_->get_logger(), 
      "Created wrench publisher for joint: " << joint_name << " on topic: " << topic_name);
  }
  ...
  const rclcpp::Duration & /*period*/)
{
  for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
    ...
    // PUBLISH WRENCH DATA IF ENABLED (NEW CODE)
    if (this->dataPtr->joints_[i].publish_wrench && 
        this->dataPtr->joints_[i].wrench_publisher) {
      geometry_msgs::msg::WrenchStamped wrench_msg;
      wrench_msg.header.stamp = time;
      wrench_msg.header.frame_id = this->dataPtr->joints_[i].name;
      wrench_msg.wrench.force.x = jointWrench->Data().force().x();
      wrench_msg.wrench.force.y = jointWrench->Data().force().y();
      wrench_msg.wrench.force.z = jointWrench->Data().force().z();
      wrench_msg.wrench.torque.x = jointWrench->Data().torque().x();
      wrench_msg.wrench.torque.y = jointWrench->Data().torque().y();
      wrench_msg.wrench.torque.z = jointWrench->Data().torque().z();
      
      this->dataPtr->joints_[i].wrench_publisher->publish(wrench_msg);
    }
  }
